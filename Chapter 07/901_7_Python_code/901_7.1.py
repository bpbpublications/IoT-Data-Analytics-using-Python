...
# Plot smoothed data against original data
plt.plot(wind_turbine_data['rtc'], wind_turbine_data['power_output_100'], label='Original Data')
plt.plot(wind_turbine_data['rtc'], rolling_avg, label='Moving Average')
plt.plot(wind_turbine_data['rtc'], wind_turbine_data['power_output_100'] - rolling_avg, label='Smoothed Data')

print("Here is the comparison of original data and smoothed data")

# Set plot labels and legend
plt.xlabel('Time')
plt.ylabel('Power Output')
plt.title('Original , Moving Average and Smoothed Data')
plt.legend()
