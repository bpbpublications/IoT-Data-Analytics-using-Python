...
wind_turbine_data = pd.DataFrame(cur.fetchall(), columns=['id', 'rtc', 'year',
'month', 'day', 'hour', 'minute',  'temperature_100', 'temperature_120',
'temperature_80',  'wind_direction_100', 'wind_direction_120', 
...
# Insert actual and predicted values into a new table
cur.execute('CREATE TABLE IF NOT EXISTS analyticszone.a_predicted_power_output (rtc TIMESTAMP, actual NUMERIC, predicted NUMERIC)')

for i in range(len(y_test)):
    cur.execute("INSERT INTO analyticszone.a_predicted_power_output (rtc, actual, predicted) VALUES (%s, %s, %s)",
                (str(y_test.index[i]), y_test.iloc[i], y_pred[i]))
